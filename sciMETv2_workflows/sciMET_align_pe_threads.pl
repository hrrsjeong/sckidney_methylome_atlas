...
if (defined $opt{'u'}) { # align swapping reads 1 and 2 and run as standard directional (faster than pbat)
	$pe_align_call = "bismark --local --unmapped -o $opt{'O'} $ref -1 $opt{'2'} -2 $opt{'1'} >> $opt{'O'}.align.log 2>> $opt{'O'}.align.log";
} else {
	$pe_align_call = "bismark --local --parallel $threads -o $opt{'O'} $ref -1 $opt{'2'} -2 $opt{'1'} >> $opt{'O'}.align.log 2>> $opt{'O'}.align.log";
}

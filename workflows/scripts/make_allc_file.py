...
input_dir_flag = sys.argv[2] #"filtered_bc_/{sample}_{Tn5}/allc_step1.complete"#sys.argv[1]
...
for sfile in flist:
    ...
    with open(fout_sh_name,'w') as fout_sh:
        fout_sh.write("zcat "+sfile+" | awk -v OFS='\\t' '{print $1,$2-1,$2,$3,$7,$4,$4+$5,"+'"1"'+"}' | bedtools subtract -a - -b "+SNP_file_path+' | cut -f 1,3,4,5,6,7,8 | grep -Fv -e "KI2" -e "GL0"  | bgzip -c > '+output_dir+'/'+sfile_out_base+'.allc.tsv.gz && tabix -b 2 -e 2 -s 1 '+output_dir+'/'+sfile_out_base+'.allc.tsv.gz\n')

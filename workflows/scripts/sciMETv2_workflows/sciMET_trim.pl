...
if (!defined $opt{'D'}) {
	...
	if (defined $opt{'u'}) {
		if ($r2_trim > 0) {
			$trim_command = "trim_galore -a $a1 -a2 $a2 --three_prime_clip_R2 $r2_trim -j $threads --paired --basename $tmp_O --retain_unpaired $opt{'1'} $opt{'2'} >> $opt{'O'}.trim.log 2>> $opt{'O'}.trim.log";
		} else {
			$trim_command = "trim_galore -a $a1 -a2 $a2 -j $threads --paired --basename $tmp_O --retain_unpaired $opt{'1'} $opt{'2'} >> $opt{'O'}.trim.log 2>> $opt{'O'}.trim.log";
		}
	} else {
		if ($r2_trim > 0) {
			$trim_command = "trim_galore -a $a1 -a2 $a2 --three_prime_clip_R2 $r2_trim -j $threads --basename $tmp_O --paired $opt{'1'} $opt{'2'} >> $opt{'O'}.trim.log 2>> $opt{'O'}.trim.log";
		} else {
			$trim_command = "trim_galore -a $a1 -a2 $a2 -j $threads --basename $tmp_O --paired $opt{'1'} $opt{'2'} >> $opt{'O'}.trim.log 2>> $opt{'O'}.trim.log";
		}
	}
	system($trim_command);
	...
	# rename to output prefix names
	#$tmp_out1 = $opt{'1'}; $tmp_out1 =~ s/\.fq\.gz$//; $tmp_out1 =~ s/\.fastq\.gz$//; $tmp_out1 .= "_val_1.fq.gz";
	#$tmp_out2 = $opt{'2'}; $tmp_out2 =~ s/\.fq\.gz$//; $tmp_out2 =~ s/\.fastq\.gz$//; $tmp_out2 .= "_val_2.fq.gz";
	$tmp_out1 = $opt{'O'}; $tmp_out1 =~ s/\.fq\.gz$//; $tmp_out1 =~ s/\.fastq\.gz$//; $tmp_out1 .= "_val_1.fq.gz";
	$tmp_out2 = $opt{'O'}; $tmp_out2 =~ s/\.fq\.gz$//; $tmp_out2 =~ s/\.fastq\.gz$//; $tmp_out2 .= "_val_2.fq.gz";
	$out1 = (split '\/', $tmp_out1)[-1];
	$out2 = (split '\/', $tmp_out2)[-1];

	if  (defined $opt{'u'}) {
		#$tmp_out1u = $opt{'1'}; $tmp_out1u =~ s/\.fq\.gz$//; $tmp_out1u =~ s/\.fastq\.gz$//; $tmp_out1u .= "_unpaired_1.fq.gz";
		#$tmp_out2u = $opt{'2'}; $tmp_out2u =~ s/\.fq\.gz$//; $tmp_out2u =~ s/\.fastq\.gz$//; $tmp_out2u .= "_unpaired_2.fq.gz";
		$tmp_out1u = $opt{'O'}; $tmp_out1u =~ s/\.fq\.gz$//; $tmp_out1u =~ s/\.fastq\.gz$//; $tmp_out1u .= "_unpaired_1.fq.gz";
		$tmp_out2u = $opt{'O'}; $tmp_out2u =~ s/\.fq\.gz$//; $tmp_out2u =~ s/\.fastq\.gz$//; $tmp_out2u .= "_unpaired_2.fq.gz";
		$out1u = (split '\/', $tmp_out1u)[-1];
		$out2u = (split '\/', $tmp_out2u)[-1];
	}
